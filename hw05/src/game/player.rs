...
impl Player {
    ...
    pub fn act(&mut self, cmd: Command) -> Result<(), ()> {
        match cmd {
            Command::Go(s) => {
                if let Ok(room) = self.find_room(s) {
                    self.location = Rc::clone(&room);
                    Ok(())
                } else {
                    Err(())
                }
            ...
            Command::Shoot(s) => {
                if let Ok(room) = self.find_room(s) {
                    if room.borrow().wumpus {
                        println!("You hit something! Or someone. You're pretty reckless that way.");
                        self.won = true;
                        room.borrow_mut().wumpus = false;
                    } else {
                        println!("Whiff!");
                    }
                    Ok(())
    ...
    pub fn victory(&self) -> bool { self.won }
